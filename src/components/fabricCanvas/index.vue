...
<script>
	...
	export default {
		...
		methods: {
			init() {
				var Fn = this.InitFabricCanvas();
				Fn["refreshImageCanvas"] = this.refreshImageCanvas;
				this.$emit("initFabricCanvas", () => {
					return Fn;
				});
			},
			//初始化canvas
			InitFabricCanvas({
				...
				//初始化canvas
				this.FabricEl = new fabric.Canvas(id, {
					backgroundColor: bg,
					isDrawingMode:
			...
			setImageCanvas() {
				if (checkDataType(this.canvasData, "String")) {
					...
					//渲染已有数据的canvas宽高
					if (this.canvasData.backgroundImage) {
						this.setCanvasWH(this.canvasData.backgroundImage);
					} else {
						this.FabricEl.setWidth(this.defaultWidth);
						this.FabricEl.setHeight(this.defaultHeight);
					}
					// 加载渲染数据
					if (this.canvasData.objects) {
						this.FabricEl.loadFromJSON(this.canvasData);
					}
				}
			},
			// 设置canvas的宽高
			setCanvasWH(
				image = { width: this.defaultWidth, height: this.defaultHeight }
			) {
				const canvasMaxWidth = this.maxWidth; // canvas最大宽度
				let imgWidth =
					image.width >= canvasMaxWidth
						? canvasMaxWidth
						: image.width;
				let imgHeight = image.height;
				let scaleX = toDecimal2(canvasMaxWidth / image.width);
				if (scaleX < 1) {
					imgWidth = image.width * scaleX;
					imgHeight = image.height * scaleX;
				}
